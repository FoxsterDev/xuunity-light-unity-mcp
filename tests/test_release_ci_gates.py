...
def all_green_runs() -> list[dict]:
    return [make_run(name) for name in gate.REQUIRED_WORKFLOWS]


class EvaluateGatesTests(unittest.TestCase):
    def test_all_required_workflows_green_passes(self) -> None:
        gates = gate.evaluate_gates(all_green_runs(), gate.REQUIRED_WORKFLOWS)

        self.assertFalse(gate.gates_blocked(gates))
        self.assertEqual([g["verdict"] for g in gates], ["pass"] * len(gate.REQUIRED_WORKFLOWS))

    def test_a_failed_required_workflow_blocks(self) -> None:
        runs = all_green_runs()
        runs[0] = make_run("Integration Tests", conclusion="failure")

        gates = gate.evaluate_gates(runs, gate.REQUIRED_WORKFLOWS)
    ...
    def test_a_missing_required_workflow_blocks(self) -> None:
        runs = [make_run(name) for name in gate.REQUIRED_WORKFLOWS if name != "Unity Package CI"]
    ...
    def test_pull_request_runs_are_not_gate_evidence(self) -> None:
        runs = [make_run(name, event="pull_request") for name in gate.REQUIRED_WORKFLOWS]

        gates = gate.evaluate_gates(runs, gate.REQUIRED_WORKFLOWS)

        self.assertTrue(gate.gates_blocked(gates))
        self.assertEqual([g["verdict"] for g in gates], ["missing"] * len(gate.REQUIRED_WORKFLOWS))

    def test_workflow_dispatch_runs_count_as_gate_evidence(self) -> None:
        runs = [make_run(name, event="workflow_dispatch") for name in gate.REQUIRED_WORKFLOWS]
...
class CheckReleaseCiGatesTests(unittest.TestCase):
    def test_fetch_errors_block_with_cannot_verify(self) -> None:
        ...
        self.assertEqual(
            ...
            ["cannot_verify"] * len(gate.REQUIRED_WORKFLOWS),
        )
