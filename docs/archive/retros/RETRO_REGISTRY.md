...
| --- | --- | --- | --- | --- |
| 2026-07-30 | `2026-07-30_reference_driven_ui_completion_and_visual_acceptance_retro.md` | Reference-image intake, deterministic UI fixtures, uGUI/prefab inspection, isolated render, visual comparison, guarded interaction, and device evidence | **new P0 reference-acceptance loop — not implemented** | Current MCP can compile, run PlayMode, and capture Game View screenshots, but it cannot bind a supplied reference image to a deterministic fixture or issue a reference-match verdict. The Flying Gift session demonstrated the false-completion risk: technical scenario passed while body text/layout/typography still did not match the supplied reference. P0 is a reference manifest plus region-aware expected/actual/overlay/diff acceptance contract and deterministic fixture envelope. P1 is uGUI-first prefab/tree validation plus isolated render. P2 is guarded prefab mutation, semantic click, and device acceptance. |
