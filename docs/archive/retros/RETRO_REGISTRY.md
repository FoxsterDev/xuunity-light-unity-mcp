...
| --- | --- | --- | --- | --- |
| 2026-09-02 | `2026-09-02_batch_summary_shape_and_compile_evidence_retro.md` | Sweep-runner verdict versus the wrapper's default compact batch payload shape, plus the missing compile-warning and rebuilt-versus-cached evidence surfaces | **intake; one P0 latent defect, two P1 evidence gaps, two P2 candidates** | `--output` defaults to `compact` for the batch subcommands, the compact projection flattens summary fields onto the payload top level and drops the nested `result_summary`, and `scripts/testing/run_multi_project.py` reads `result_summary` only from `payload["result_summary"]` (`:381`) with `unity_outcome`/`transport_outcome` sourced from it alone (`:419-422`) while five neighbouring lane/license fields already have a payload fallback (`:398-417`). A passing GUI-fallback run (`status: passed`, `6/6`, `validation_evidence: unity_mcp`, `post_settle_compile_trust_class: confirmed`) therefore surfaced as `failed_wrapper_unity_unproven` with `total=0` beside `succeeded=true`, and exit 1. Not a race: compact is the default, so the runner's happy path depends on the non-default shape, and no test feeds the runner the compact projection. This is the 2026-06-07 verdict taxonomy colliding with the 2026-06-02/2026-06-11 compact-envelope work. P1: the compile-matrix artifacts carry `error_count`/`errors` but no warning fields at all (zero `warn` occurrences), so a warning-cleanup task cannot be answered from any compact surface; and `compiled_assembly_count` is compile-set size, not rebuild evidence (one run reported 85/82 per config against 374 real `Csc` invocations and 330 `[CacheHit …]`). First archive sighting of the cached-versus-rebuilt gap. P2: `--output` is parsed but never consumed, and the full-nested payload branch is unpinned. Nothing implemented yet. |
