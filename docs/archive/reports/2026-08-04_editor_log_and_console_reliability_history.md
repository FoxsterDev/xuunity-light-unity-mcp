...
| 2026-08-19 | `0.3.58` | Anchored `since=playmode_start` scope of 1,789,547 bytes searched only its last 500,000 (`EDITOR_LOG_GREP_MAX_CHARS`), so a boot-time line in the head was missed and reported as `match_count: 0`. The arithmetic at the truncation boundary is correct — the gap is that a truncated zero-match is inconclusive, not negative, and the cut keeps the tail when an anchor asks for the head | **open** — retro `2026-08-19_anchored_scope_truncation_and_verdict_field_ranking_retro` proposes inconclusive ranking plus an anchor-adjacent window | absence ≠ evidence (semantic, not offset arithmetic) |
