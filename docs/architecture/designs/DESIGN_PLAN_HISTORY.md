...
Date: `2026-05-26`
Last re-review: `2026-07-27` (SDK rollout plan re-verified against released `v0.3.47` and current source)
...
| --- | --- | --- | --- |
| 2026-07-12 | `XUUNITY_MCP_SDK_ROLLOUT_GATE_IMPLEMENTATION_PLAN_2026-07-12.md` | `P0.1 + P0.2a complete in current source; broader plan open` | The generated-diff guard shipped its Git-tracked slice in `v0.3.45`; later current source adds structure-aware comparison, fingerprint-bound Git-untracked baselines, and automatic report registration. Current source now also makes Android `unity.edm4u.resolve` fail closed unless Android is active and marks freshness unproven. The next false-verdict slice is the typed engine-driven stable-hash/new-coordinate oracle. GUI admission, batch resolve, and portfolio orchestration remain later phases. |
