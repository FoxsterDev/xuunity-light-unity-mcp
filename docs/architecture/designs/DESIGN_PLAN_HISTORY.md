...
| --- | --- | --- | --- |
| 2026-07-12 | `XUUNITY_MCP_SDK_ROLLOUT_GATE_IMPLEMENTATION_PLAN_2026-07-12.md` | `P0.1 complete in current source; broader plan open` | The generated-diff guard shipped its Git-tracked slice in `v0.3.45`; current source adds structure-aware comparison, fingerprint-bound Git-untracked baselines, and automatic artifact registration for published pass/fail reports. The next false-verdict slice is typed Android resolver target enforcement plus engine-driven output freshness. GUI admission control, batch resolve, and portfolio orchestration remain later phases. |
