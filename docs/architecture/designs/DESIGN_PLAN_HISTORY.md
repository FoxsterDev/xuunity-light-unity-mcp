...
| --- | --- | --- | --- |
| 2026-07-30 | `XUUNITY_MCP_REFERENCE_DRIVEN_UI_ACCEPTANCE_DESIGN_2026-07-30.md` | `P0.1 implemented and host-validated; P0.2/P1.x/P2.x planned` | From the 2026-07-30 reference-driven UI retro. Shipped the reference contract and comparator: `unity_ui_reference_register` / `_validate` / `_compare` plus `ui-reference-*` CLI commands, `xuunity.ui-reference.v1` manifests with hash-pinned expected images, declared-mask policy, and per-lane acceptance. Acceptance is tolerance-based similarity on a resolution-independent cell grid (exact box average + local contrast + layout bounding box), never pixel equality, so a Game View capture at a different resolution than the reference is valid input; orientation/aspect mismatch is refused as `comparison_not_comparable` with no score. Verdicts distinguish `passed` / `failed` / `blocked` / `pending_lanes` / `pending_manual_style` and report `decision_ready` separately. Validation: 517 host tests green (41 new), production-scale check passed a 1080x2400 capture against a 1440x3200 reference at 0.997 similarity in 0.7 s and failed the wrong-art/missing-body case with per-region numbers. Open: fixture contract (P0.2), prefab validation + uGUI semantic tree (P1.1), isolated renderer (P1.2). |
