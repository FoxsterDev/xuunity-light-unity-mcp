...
for MCP-capable AI agents. The current released source line is `v0.3.48`.
...
Latest release and current-source validation for `v0.3.48`:

| Area | Evidence | Result |
| --- | --- | --- |
| Package metadata | `packages/com.xuunity.light-mcp/package.json` | `name=com.xuunity.light-mcp`, `version=0.3.48`, `unity=2021.3`, no hard Test Framework dependency |
| Host Python tests | Focused protocol/parity/atomicity/launcher suite plus full discovery | Focused current-source coverage passes `77/77`. The full host suite passes `476/476` with 13 expected platform skips. |
| Compact MCP envelopes | Changelog and regression coverage for `0.3.32`-`0.3.48` | Scenario decision verdicts, compact operation/readiness/status summaries, authoritative post-settle compile/test/refresh fields, editor-log identity, scenario step-payload opt-ins, PlayMode already-playing stale-risk summaries, deterministic scene-open setup, opt-in compact batch helper output, safer `Editor.log` console grep/tail defaults, compact transport/idle timeout errors, compile-first post-change validation, lane-agnostic GUI-fallback compile evidence, and requested-filter zero-match verdicts are documented with full-payload recovery. |
...
| Public site checks | `scripts/testing/run_site_ui_checks.sh` | Public site Playwright checks passed for `v0.3.48`: `42/42`. |
...
| Git tag visibility | Remote Git refs | Release tag `v0.3.48` is the current Git UPM release target; remote publication requires an authenticated push. |
...
{
  "dependencies": {
    "com.xuunity.light-mcp": "https://github.com/FoxsterDev/xuunity-mcp.git?path=/packages/com.xuunity.light-mcp#v0.3.48"
  }
}
```
...
- `prodmode` pins the Unity project to the published release tag that matches
  the package version, for example `#v0.3.48`.
