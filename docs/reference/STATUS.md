...
for MCP-capable AI agents. The current released source line is `v0.3.63`.
...
Latest release and current-source validation for `v0.3.63`:

| Area | Evidence | Result |
| --- | --- | --- |
| Package metadata | `packages/com.xuunity.light-mcp/package.json` | `name=com.xuunity.light-mcp`, `version=0.3.63`, `unity=2021.3`, no hard Test Framework dependency |
...
| `v0.3.63` package lanes on both Unity lines | Clean devmode projects driven through the public package self-test and contract matrix | Unity `2022.3.67f2` and `6000.0.58f2` each pass package EditMode `91/91` and dependency-free PlayMode `5/5`, with authoritative post-settle compile green. |
...
| Public site checks | `scripts/testing/run_site_ui_checks.sh` | Public site Playwright checks passed for `v0.3.63`: `42/42`. |
...
| Git tag visibility | Remote Git refs | Release tag `v0.3.63` is the current Git UPM release target; remote publication requires an authenticated push. |
...
- `prodmode` pins the Unity project to the published release tag that matches
  the package version, for example `#v0.3.63`.
