...
for MCP-capable AI agents. The current released source line is `v0.3.49`.
...
Latest release and current-source validation for `v0.3.49`:

| Area | Evidence | Result |
| --- | --- | --- |
| Package metadata | `packages/com.xuunity.light-mcp/package.json` | `name=com.xuunity.light-mcp`, `version=0.3.49`, `unity=2021.3`, no hard Test Framework dependency |
...
| Public site checks | `scripts/testing/run_site_ui_checks.sh` | Public site Playwright checks passed for `v0.3.49`: `42/42`. |
...
| Git tag visibility | Remote Git refs | Release tag `v0.3.49` is the current Git UPM release target; remote publication requires an authenticated push. |
...
{
  "dependencies": {
    "com.xuunity.light-mcp": "https://github.com/FoxsterDev/xuunity-mcp.git?path=/packages/com.xuunity.light-mcp#v0.3.49"
  }
}
```
...
- `prodmode` pins the Unity project to the published release tag that matches
  the package version, for example `#v0.3.49`.
