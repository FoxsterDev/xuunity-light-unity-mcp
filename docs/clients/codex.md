...
This guide is pinned to XUUnity release `v0.3.48` from the canonical repository
`https://github.com/FoxsterDev/xuunity-mcp`. Use the matching release README:
`https://github.com/FoxsterDev/xuunity-mcp/blob/v0.3.48/README.md`.
...
- Requested package release: <v0.3.48>
...
- Required live proof after restart: <server listed, tools listed, unity_status_summary healthy with mcp_server_info.version=0.3.48>
...
v0.3.48 source through the native wrapper instead of executing an old helper or
requiring Git Bash:
...
`.source_root` with v0.3.48. Do not execute stale helper files. Refresh them
from the approved v0.3.48 source after the preflight review is approved.
...
the installed version/source against v0.3.48 before executing it for this setup.
...
1. `unity_status_summary` and require `mcp_server_info.version=0.3.48`
