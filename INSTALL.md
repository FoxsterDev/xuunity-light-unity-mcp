...
- release-pinned README:
  `https://github.com/FoxsterDev/xuunity-mcp/blob/v0.3.65/README.md`
...
- If a host helper directory already exists under `~/.codex-tools`,
  ...
  have been checked against v0.3.65 and stale files have been refreshed from the
...
```text
Set up XUUnity Light Unity MCP release v0.3.65 from
https://github.com/FoxsterDev/xuunity-mcp for /path/to/UnityProject. Follow
https://github.com/FoxsterDev/xuunity-mcp/blob/v0.3.65/README.md. Before executing
an existing helper, compare its version and .source_root with v0.3.65 and refresh
...
unity_status_summary. Require mcp_server_info.version=0.3.65 in that live result.
...
2. compare the requested v0.3.65 release with the manifest package pin, installed
   helper version/source, and current client launcher before executing an
   existing helper
3. use `setup-plan` from the approved v0.3.65 source before any mutation
...
the requested release. For an install or upgrade to v0.3.65, inspect before use
and refresh from the approved v0.3.65 source after approval.
...
- Requested package release: <v0.3.65>
...
- Required live proof after restart: <server listed, tools listed, unity_status_summary healthy with mcp_server_info.version=0.3.65>
...
{
  "dependencies": {
    "com.xuunity.light-mcp": "https://github.com/FoxsterDev/xuunity-mcp.git?path=/packages/com.xuunity.light-mcp#v0.3.65"
  }
}
```
...
or perform a v0.3.65 upgrade: an old refresh launcher can remain pinned to its
old source checkout. Run `setup-plan` from the approved v0.3.65 source checkout,
...
On hosts where Unity Hub uses a dynamic licensing pipe, do not inspect or copy
raw process arguments manually. `ensure-ready --open-editor` discovers only
live Hub-owned `Unity.Licensing.Client` children. One candidate is forwarded
with redacted provenance; zero candidates returns a typed Hub/sign-in action
when required; multiple candidates fail closed. A Terms, sign-in, or activation
condition is reported as `terms_or_activation_ui_required`, while an available
but unforwarded channel is `hub_ipc_available_but_not_forwarded`.

For a shareable diagnostic without raw channels, process commands, project
identity, or unbounded logs, run:

```bash
bash xuunity_light_unity_mcp.sh diagnostic-retro-bundle \
  --project-root /path/to/UnityProject
```
...
For package-level verification after upgrading to `v0.3.65`, run:
