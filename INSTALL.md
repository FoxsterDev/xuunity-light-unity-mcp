...
- release-pinned README:
  `https://github.com/FoxsterDev/xuunity-mcp/blob/v0.3.70/README.md`
...
- If a host helper directory already exists under `~/.codex-tools`,
  ...
  have been checked against v0.3.70 and stale files have been refreshed from the
...
```text
Set up XUUnity Light Unity MCP release v0.3.70 from
https://github.com/FoxsterDev/xuunity-mcp for /path/to/UnityProject. Follow
https://github.com/FoxsterDev/xuunity-mcp/blob/v0.3.70/README.md. Before executing
an existing helper, compare its version and .source_root with v0.3.70 and refresh
...
unity_status_summary. Require mcp_server_info.version=0.3.70 in that live result.
...
2. compare the requested v0.3.70 release with the manifest package pin, installed
   helper version/source, and current client launcher before executing an
   existing helper
3. use `setup-plan` from the approved v0.3.70 source before any mutation
...
the requested release. For an install or upgrade to v0.3.70, inspect before use
and refresh from the approved v0.3.70 source after approval.
...
- Requested package release: <v0.3.70>
...
- Required live proof after restart: <server listed, tools listed, unity_status_summary healthy with mcp_server_info.version=0.3.70>
...
or perform a v0.3.70 upgrade: an old refresh launcher can remain pinned to its
old source checkout. Run `setup-plan` from the approved v0.3.70 source checkout,
...
For package-level verification after upgrading to `v0.3.70`, run:
